...
The result of the correlation analysis on transcripts level counts is shown in Figure \@ref(fig:plotTranscripts). The Pearson correlation coefficient is $r = `r pearsonTranscripts`$, the Spearman correlation is $\rho = `r spearmanTranscripts`$.

```{r plotTranscripts, fig.cap = paste0("Correlation plots for transcript-level read count analysis with (A) linear axes and (B) log2-transformed axes. Pearson correlation $r = ", sprintf("%.2f", pearsonTranscripts), "$; Spearman correlation $\\rho = ", sprintf("%.2f", spearmanTranscripts), "$; the blue line is the linear model-fitted regression line $count\\_bulk \\sim count\\_st$. Genes with missing data for counts have received a dummy value of -1000 (only shown in panel A).")}
...
```{r plotGenes, fig.cap = paste0("Correlation plots for gene-level read count analysis with (A) linear axes and (B) log2-transformed axes. Pearson correlation $r = ", sprintf("%.2f", pearsonGenes), "$; Spearman correlation $\\rho = ", sprintf("%.2f", spearmanGenes), "$; the blue line is the linear model-fitted regression line $count\\_bulk \\sim count\\_st$. Genes with missing data for counts have received a dummy value of -1000 (only shown in panel A).")}
