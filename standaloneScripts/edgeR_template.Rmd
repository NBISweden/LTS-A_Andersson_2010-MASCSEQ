...
data_sets = list(
  tet1 = "/path/to/043_Tetra_C1_new_index_stdata.tsv",
  tet2 = "/path/to/043_Tetra_D2_new_index_stdata.tsv",
  tet3 = "/path/to/043_Tetra_E1_new_index_stdata.tsv"
...
Next we perform a DGEA. This is done pairwise, so we first create a list with all pairwise combinations between the groups. We use edgeR's exact test and print the result as a scrollable datatable.
...
htmltools::tagList(
  lapply(comparisons, 
         function(cmp){
           ...
           #top_genes_mean=meanDge$table[meanDge$table$p.adjust<=0.05,]
           meanDge$table %>%
