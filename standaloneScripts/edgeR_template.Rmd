...
The variation of the read counts can be though of as comprising a *technical* component (measurement errors etc.) and a *biological* component. The *biological coefficient of variation* (BCV) is estimated from the data using an empirical Bayes approach, and is used as the *dispersion parameter* in the negative binomial distribution.
...
This is the code for reading the data. It mimics what is done in your R-script using a loop over the Tetrahymena data-sets. A minor difference is that I save the counts data in a list indexed by the selected data set names (this simplifies later comparisons).
...
# NB! change paths to fit where your inpout files are!
data_sets = list(
...
for(group in names(data_sets)){                                              # for each "group" 
  counts[[group]] = as.matrix(read.delim(data_sets[[group]], row.names = 1)) # read the data and convert to matrix
}
```
...
createDgeList <- function(dataList){
  ...
  target = do.call(rbind, lapply(names(dataList), function(x) 
    data.frame(replicates = paste(x, rownames(dataList[[x]]), sep = "."),
               group = as.factor(x))))
  ...
  mergedData = t(do.call(rbind, lapply(dataList, function(x)
    x[,commonGenes])))
...
In our first approach, we will treat each spot as a biological replicate. A problem is that the read counts might be very low, which is suboptimal for a proper Differential Gene Expression Analysis (DGEA). For our use-case, e.g., *"Does the three Tetrahymena replicates differ substantially?"*, it might provide some information, but the low read counts will be a problem here as well; edgeR will surely issue a lot of warnings when trying to log-transform count values that are zero. It will not be a publishable analysis, I believe.

### Convert to edgeR format and setup for DGEA

We first concatenate and convert the data into a a single matrix in edgeR format, including all the different groups. EdgeR then uses an Empirical Bayes approach to estimate the dispersion function of the Negative Binomial distribution, assumed to underlying the residual variation, and plot the obtained dispersion distribution.
...
# Create a DGElist of all spots in the three datasets using the funciton defined above
...
Next we perform a DGEA. This is done pairwise, so we first create a list with all pairwise combinations between the groups. We use edgeR's exact test and print the result as a scrollable datatable, sorted firstly by in ascending adjusted P-value and secondly by descending logFC

```{r meanDge, results="asis"}
# creates a list of all possible comparisons among datasets
...
htmltools::tagList(
  lapply(comparisons, 
         function(cmp){
           ...
           meanDge$table$p.adjust = p.adjust(meanDge$table$PValue, method=params$adjust)
           ...
           # We order the gens by their adjusted P-values
           meanDge$table[order(meanDge$table$p.adjust, -meanDge$table$logFC),] %>%
             ...
             formatSignif(columns=c("logFC", "PValue","p.adjust"), digits=3)
...
```{r mergedDGE, echo=FALSE}
# A function that creates indata for edgeR analysis of group1 (with data1) and group2 (with data2)
# returns a DGElist  ready for use with edgeR
mergeData <- function(data1){
  ret = colSums(data1)
  rownames(ret)=group1
  return(ret)
}
...
mergedCounts = lapply(names(counts), function(x)
  matrix(colSums(counts[[x]]),nrow=1, dimnames = list(x, colnames(counts[[x]]))))
...
# Make sample-indexed color vectors (if more than 8 groups, you need to add more colors)
...
plotMDS(dgeMergedData, dim.plot=c(1,2), asp=2, 
        bg= bgcols[dgeMergedData$samples$group], 
        pch=21, cex=3)
# Since, we only have 3 groups
# plotMDS(y, dim.plot=c(1,3), asp=2, 
#         bg= bgcols[dgeMergedData$samples$group], 
#         pch=21, cex=3)
plot.new()
