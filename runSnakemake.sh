...
if [[ "$(mamba info -e | awk -v var=$snakemake '{if($1==var)print "found"}')" != "found" ]]; then
    mamba env create --file $DIR/envs/snakemake.yaml
fi
...
#     # To run on cluster, a cluster config file is needed. If not present, copy
#     # from workflow base directory. Update the contact email at the same time.
#     if [[ ! -f cluster.yaml ]]; then
# 	echo "No cluster.yaml found in present directory"
...
# 	read email
# 	awk -v email=$email '{sub("your@email", email, $0); print $0}' $DIR/cluster.yaml > cluster.yaml
# 	echo "cluster.yaml copied with email updated. Your can make further changes to the file manually."
#     fi
